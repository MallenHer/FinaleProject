...
const url = "http://localhost:3000/projects" 
  ...
  class Projects extends React.Component {
    ...
    getProjectData = () => {
        axios.get("http://localhost:3000/allProjects",  {withCredentials: true })
            .then(res => {
                console.log(res)
    ...
    render() {
        const { profileInfo, projects } = this.state;
        console.log(profileInfo) 
        let projectos = projects.map((project, index) => { 
            return(
                ...
                //     <h1 className="heading">Meet The Projects</h1>
                    ...
                    <p className="job-title">{project.nameproject}</p>
                    <p className="about">{project.sector}</p>
